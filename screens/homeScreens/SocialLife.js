...
export default function SocialLife() {
  ...
  const { isLoading, currentUser } = useSelector((state) => state.authentication);
  useEffect(() => {
  ...
  return (
    <ScrollView style={{ flex: 1 }}>
      {isLoading ? (
        ...
        <>
          <View style={[childStyle.imageContainer, { width, height: height / 3 }]}>
            <Image
          ...
          <View style={childStyle.buttonContainer}>
            ...
            >
              ...
              <CoffeeSvg fill={appColors.iconInActive} />
            </ButtonComponent>
            <ButtonComponent
              style={childStyle.childButtons}
